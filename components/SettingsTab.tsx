...
export default function SettingsTab() {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {saving && (
        <div className="fixed top-4 right-4 bg-green-500 text-white px-3 py-1.5 rounded-lg text-xs shadow z-50">
          已保存
        </div>
      )}
      ...
      <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200 mb-4">
        ...
        {provider === "custom" && (
          <div className="space-y-2">
            <input
              type="text" placeholder="API Base URL" value={customUrl}
              onChange={(e) => setCustomUrl(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs"
            />
            <input
              type="password" placeholder="API Key" value={customKey}
              onChange={(e) => setCustomKey(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs"
            />
            <input
              type="text" placeholder="Model 名称" value={customModel}
      ...
      <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200">
        ...
        <p className="text-sm text-gray-500">英语学习助手 v0.3.0</p>
